...
try {
  const { values } = parseArgs({ options: { input: { type: 'string' }, output: { type: 'string' },
    'install-dir': { type: 'string' }, overwrite: { type: 'boolean', default: false } } });
  ...
  const container = await createFromSpecification(JSON.parse(await readFile(input, 'utf8')), dirname(input));
