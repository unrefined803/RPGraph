...
export function validateAssistantCharacter(character: Character) {
  ...
  for (const [app, account] of Object.entries(character.apps ?? {})) {
    ...
    if (!account.username.trim() || (app !== 'whatsup' && !/^[a-zA-Z0-9._-]+$/.test(account.username))) {
      throw new Error(`${app}: enter a valid username (letters, numbers, dots, underscores or hyphens).`);
    }
    if (!account.displayName.trim() || account.displayName.length > 60 || account.bio.length > 500) {
      throw new Error(`${app}: enter a display name of 1–60 characters and a bio of at most 500 characters.`);
    }
    if (app === 'matchme' && !character.apps?.matchme?.profile) throw new Error('MatchMe requires a complete profile.');
    const profile = app === 'matchme' ? character.apps?.matchme?.profile : undefined;
    if (profile && (profile.interests.length > 150 || profile.name !== account.displayName || profile.bio !== account.bio)) {
...
export function parseCharacterAssistantResult(text: string, current: Character) {
  ...
  for (const operation of response.patch) {
    ...
    if (!allowed || parts.some((part: string) => ['__proto__', 'prototype', 'constructor', 'dataUrl', 'sampleDataUrl', 'accountId'].includes(part))) {
