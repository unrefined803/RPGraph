...
export function RpStorybookV1NodeCard({ id, data }: NodeProps<WorkflowNode>) {
  const nodeBodyRef = useNodeLayoutSync(id);
  const { loadStorybookFile, openStorybookCreator } = useNodeActions();
    ...
    <div className={`workflow-node rp-storybook-node${runStateClassName(data)}`} ref={nodeBodyRef}>
      ...
      <div className="storybook-actions">
        <button
          ...
          type="button"
          onClick={() => void loadStorybookFile(id)}
        >
          Load Storybook
        </button>
