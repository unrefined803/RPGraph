...
Before a Prompt Switch output can create a Fotogram or OnlyFriends message, both participants are resolved by exact app handle or display name. Known Storybook characters without an account in that app and unknown handle-only identities are blocked. The node discards the invalid message and retries once with a small private correction context; a second invalid result remains blocked. Unknown full display names are still accepted as new story NPCs.
