...
A prompt slot can additionally be split into a planning step and a main step with the standalone marker lines `@step:planning` and `@step:main` (case-insensitive). The planning section runs first as a separate LLM pass; everything the planning LLM sees is visible, editable prompt text inside that section, nothing is appended from code. The multistep workflow's planning instruction asks for a rough direction plus one to three bullets: certain developments are plain statements, uncertain ones are either/or rolls that state the success outcome with a chance like `(80%)` and an `otherwise: ...` part for failure. The app dices every bullet that carries a percentage (high roll good) and replaces the percentage with one of four binding results — clear success, success, failed (the otherwise-part happens), badly failed — without exposing the raw roll. The planning pass can also consume pre-reply image actions itself: an action request in the planning output runs the follow-up and the action, then planning reruns with the result, so a planned phone image is fetched or created before the main pass and is never rolled; the multistep prompts request phone images exclusively in the planning step, and the main step only attaches the imageId named in the plan. The rolled plan replaces every `@output:planning` token in the main section (or is prepended to its top without one), so the prompt author writes their own lead-in text around the token. Afterwards the normal pass — including actions, commands, and streaming — runs unchanged; prompts without a `@step:planning` marker keep the classic single-pass behavior. The splitting and dice logic lives in `src/nodes/shared/promptSteps.ts`; the multistep Normal RP and Messenger prompts use this planning step.
