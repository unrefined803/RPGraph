...
A prompt slot can additionally be split into two steps with standalone marker lines such as `--- Step 1: Planning` and `--- Step 2: Main Prompt` (case-insensitive, at least three dashes, free title). The Step 1 text runs first as a separate planning pass: a fixed instruction makes the LLM output a short bullet list of what can happen next, each bullet ending with a probability like `(80%)`. The app then dices every bullet automatically (high roll good) and replaces the percentage with one of four outcomes — great success, success, failure, epic fail. The rolled plan is injected into the Step 2 main prompt at every `@plan:output` token (or prepended to its top without one) as a binding outline before the normal pass — including actions, commands, and streaming — runs unchanged. Prompts without a Step 1 marker keep the classic single-pass behavior. The splitting and dice logic lives in `src/nodes/shared/promptSteps.ts`; the bundled Normal RP and Messenger prompts use this planning step.
